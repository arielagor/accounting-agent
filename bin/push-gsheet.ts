 ...
 * run). Creates the Sheet on first run (id saved in accounting/.gsheet.json), then
 * clears + rewrites the tabs: YTD Summary, Business Expenses YTD, Per-Project YTD,
 * To Review. Wired into the close so every close keeps the Sheet current.
 *
...
import { getAccessToken, hasToken } from "../src/lib/google-auth.js";
import { createSpreadsheet, writeTabs, spreadsheetUrl, type Cell } from "../src/lib/gsheets.js";
import { log, warn, error } from "../src/lib/log.js";

const root = join(dirname(fileURLToPath(import.meta.url)), "..");
const CONFIG = join(root, ".gsheet.json");
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!cfg.spreadsheetId) {
      const created = await createSpreadsheet(token, `Accounting — ${tenant} (live)`);
      cfg.spreadsheetId = created.spreadsheetId;
      writeFileSync(CONFIG, JSON.stringify(cfg, null, 2));
      log(`created live Sheet: ${created.url}`);
    }
    const tabs = await buildTabs(sql, tenant, year);
    await writeTabs(token, cfg.spreadsheetId, tabs);
    log(`live Sheet updated: ${spreadsheetUrl(cfg.spreadsheetId)}`);
    process.stdout.write(spreadsheetUrl(cfg.spreadsheetId) + "\n");
  } catch (e) {
...
main().catch((e) => { error("push-gsheet fatal:", e instanceof Error ? e.message : String(e)); process.exit(1); });
